...
def train(args, model, device, train_data, dev_data, test_data, processor):
    ...
    for i_epoch in trange(0, int(args.epochs), desc="Epoch", disable=False):
                ...
                if isinstance(audio_list[0], torch.Tensor):
                    ...
                    for a in audio_list:
                        if isinstance(a, dict):
                            ...
                            audio_tensors.append(torch.as_tensor(a, dtype=torch.float32))
        ...
        if dev_acc > max_acc:
            ...
            torch.save(model_to_save.state_dict(), os.path.join(path_to_save, 'model_500.pt'))
            ...
            if patience_counter >= int(args.patience):
                logging.info(f"--- EARLY STOPPING TRIGGERED! Stopping training at epoch {i_epoch} ---")
                break
